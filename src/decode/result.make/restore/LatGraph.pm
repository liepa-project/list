...
#X                                                                  X
#X LatGraph.pl, Copyright(C) Gailius Ra�kinis, 2020                 X
#X                                                                  X
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
#return;
   ...
   for(my $i=0; $i<scalar @merges; $i+=2) {
      ...
      for(my $j=$from; $j<$to; $j++) {
         ...
         if ($self->multi_income($self->{_in}->{$n2}, $edges[$j]) == 1) { # reikia neskaiciuoti nauj� ry�i�
#         if ((scalar @{$self->{_in}->{$n2}}) > 1) { # reikia neskaiciuoti nauj� ry�i�
            $firstin = $j;
            last;
            }
         }
...
sub extend_chain {
   ...
   #my $proc_done = 0; # becomes 1 to inhibit multiple recursion calls (2 t�kst. 2 m. + 2 t�kst. 2 met�)
   my $rc;
...
#   if ($may_teminate_here == 1 && (scalar @$words_ref) > 1 && (!defined $rc || $rc ne 'M') ) {
      ...
      #$proc_done = 1; # never get here again in the same $i loop with a different stop (-) word (e.g. m. or met�)
...
# proced�ros tikslas atstatyti skai�ius pagal skaitinius komponentus
sub connect_nums {
   my ( $self ) = @_;
...
# proced�ros tikslas suma�inti briaun�/�od�i� skai�i� redaktoriui
sub reduce_out {
   my ( $self ) = @_;

   # sur��iuojame briaunas pagal laik� ir �od� 
...
#  print join(' ', keys @{$self->{_e}} )."\n\n";
   ...
   # naikiname smarkiai persidengian�ias briaunas
